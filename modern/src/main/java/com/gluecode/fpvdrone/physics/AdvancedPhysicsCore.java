// Ported from minecraft-fpv/mfpv-client, GPL-3.0, 2026-09-13.
...
public class AdvancedPhysicsCore implements IPhysicsCore {
  private float lerp(float t, float a, float b) { return a + t * (b - a); }
    ...
    private final float rads = (float) (Math.PI / 180);
    private final float degs = (float) (180 / Math.PI);

    private final float refTemp = 293.15f;

    private float refResistance = 0.030f;
    private float heatCap = 1f;
    private float inertia = 1f;
    private float motorSurfaceArea = 1f;
    private float[] internalEnergy = new float[4];
    private float[] temp = new float[4];
    private float temp1 = refTemp;
    private float temp2 = refTemp;
    private float temp3 = refTemp;
    private float temp4 = refTemp;
    private float[] motorVel = new float[4];
    private float[] motorPos = new float[4];
    private Vector3f velocity = new Vector3f(0, 0, 0);

    private Vector3f droneLook = new Vector3f(0, 0, 1);
    private Vector3f droneUp = new Vector3f(0, 1, 0);
    private Vector3f droneLeft = new Vector3f(1, 0, 0);
    ...
    private void staticStep(float dt) {
        ...
        float sag = lerp(FastMath.abs(throt), 4.0f, 3.6f);
    ...
    private Vector3f getMotorToProp(
            float rotationAngle,
     ...
     * */
    private Vector3f getWorldPropVelocity(
            Vector3f worldDroneUp,
    ...
    private Vector3f getFreeVelRelPropPlanar(
    ...
    private float getPropDragCoefficient(float aoa) {
    ...
    private float getPropLiftCoefficient(float aoa) {
     ...
     * */
    private Vector3f getPropDragForce(
            Vector3f freeVelRelPropPlanar,
    ...
    private Vector3f getPropLiftForce(
            Vector3f freeVelRelPropPlanar,
            Vector3f motorToProp,
    ...
    private Vector3f[] getTotalForcesOnProp(
            int motorNumber,
            float rotvel
    ) {
    ...
    private int getPoles(float motorWidth) {
    ...
    private int getSlots(float motorWidth) {
        if (motorWidth >= 0.018f) {
            return 12;
        } else {
            return 9;
    ...
    private float getSlotW(float R, float slots) {
        return 2f * FastMath.PI * R / slots - 0.0005f;
    }

    private float getMagW(float R, float poles) {
        return 2f * FastMath.PI * R / poles - 0.0005f;
    }

    private float getPoleMagneticMoment(float R, float h, float poles) {
    ...
    * */
    ...
    private void setHeatCap(float motorMass) {
        heatCap = PhysicsConstants.motorSpecificHeatCapacity * motorMass;
    }

    private void setMotorSurfaceArea() {
        float motorWidth = PhysicsState.getMotorWidth();
        float motorHeight = PhysicsState.getMotorHeight();
        float R = (motorWidth ) * 0.5f;
    ...
    private void setMotorInertia(int nBlades, float propDiameter) {
        float motorWidth = PhysicsState.getMotorWidth();
    ...
    private float getHotResistance(float temp) {
    ...
    private float getCurrent(float resistance, float vbat, float rotvel) {
    ...
    //  * */
    //  private float getSlotMagneticMoment(float current, float R, float h, float slots, float N) {
    //    float slotW = getSlotW(R, slots);
    //    float slotArea = slotW * h;
    //    return N * current * slotArea;
    //  }

    private float getTorque(float current, float kv) {
        return current / kv;
    }

    private float getRotDrag(
            int motorNumber,
            float rotvel,
            float torque
    ) {
    ...
    private float getInternalEnergyLost(float temp, float timeStep) {
        //    global refTemp;
        //    global heatCap;
        //    global heatTransfer;
        //    global surfaceArea;
    ...
    private float getInternalEnergyGained(
            float vbat,
            float rotvel,
            float timeStep
    ) {
    ...
    private float getNextInternalEnergy(
            int motorNumber,
    ...
    private float getNextTemp(float nextInternalEnergy) {
        float nextTemp = nextInternalEnergy / heatCap + refTemp;
        return nextTemp;
    }

    private void rotateDrone(float elapsed) {
